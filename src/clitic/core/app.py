...
class App(TextualApp[Any]):
  ...
  # Load base styles from package
  CSS_PATH = str(files("clitic.styles").joinpath("base.tcss"))
  ...
  def get_plugin_for_content(self, content_type: str, content: str) -> ContentPlugin | None:
    ...
    """
    matching_plugins = [
      plugin for plugin in self._plugins if plugin.can_render(content_type, content)
    ]
